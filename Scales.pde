...
void draw() { 
  for (int y = 700; y > -100; y = y - 60) { 
    for (int x = 700; x > -20; x = x - 65) { 
      scale(x, y); 
      x = x + 33;
    } 
    y = y+35;
...
void scale(int x, int y) { 
  stroke(1); 
  fill((int)((Math.random()*51)*5), 0, (int)((Math.random()*51)*5)); 
  bezier(x, y, x, y+25, x, y+25, x-50, y+50); 
  bezier(x, y, x-25, y, x-25, y, x-50, y+50); 
  noStroke(); 
  fill((int)((Math.random()*51)*4 + 51), 0, (int)((Math.random()*51)*4 + 51));
